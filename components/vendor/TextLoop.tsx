...
/**
 * TextLoop — vendored from React Bits (https://reactbits.dev).
 *
 * Copyright (c) 2026 David Haz. MIT + Commons Clause License Condition v1.0.
 * Used here as part of a website, which the licence permits; the components
 * themselves are not sold, sublicensed or redistributed.
 *
 * Source: DavidHDev/react-bits, src/ts-tailwind/TextAnimations/TextLoop.
 * Unmodified apart from this header. It depends only on gsap, which this
 * project already uses, so it adds no new package.
 *
 * Type text on an SVG path and scroll it along that path forever. Two copies
 * (head and tail) are offset by exactly the path length so the seam never
 * shows. It already honours prefers-reduced-motion by not starting the tween.
 */
